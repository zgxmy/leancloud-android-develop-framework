...
public class LeanCloudFileResponse extends SaveCallback {
    private final static String TAG = "LeanCloudFileResponse";
    ...
    public void done(AVException e) {
        if (e==null){
            ...
            if (position == fileCount-1){
                LeanCloudUtils.printLog("--------LeanCloudFileResponse----------sendSuccessMessage---------");
                handler.sendSuccessMessage(request.getRequestId(),null);
            }
        }else {
            LeanCloudUtils.printLog("--------LeanCloudFileResponse----------sendFileFailureMessage---------");
