...
public class LeanCloudHandler implements LeanCloudHandlerInterface {
    private final static String TAG = "LeanCloudHandler";
        ...
        switch (message.what) {
            case START_MESSAGE:
                ...
                if (response != null) {
                    ...
                    listener.onFailure(0, new AVException( "请求失败",new Throwable()));
                }
                break;
    ...
    public void sendStartMessage(int requestId) {
        LeanCloudUtils.printLog("--------LeanCloudHandler----------sendStartMessage---------");
        sendMessage(obtainMessage(START_MESSAGE, new Object[]{requestId}));
    }

    @Override
    public void sendFailureMessage(int requestId, AVException e) {
        LeanCloudUtils.printLog("--------LeanCloudHandler----------sendFailureMessage---------");
        e = LeanCloudUtils.getExceptionWithMessage(e);
    ...
    public void sendSuccessMessage(int requestId, List<AVObject> list) {
        LeanCloudUtils.printLog("--------LeanCloudHandler----------sendSuccessMessage---------");
    ...
    public void sendFileProgressMessage(int requestId,Integer integer,int position) {
        LeanCloudUtils.printLog("--------LeanCloudHandler----------sendFileProgressMessage---------");
    ...
    public void sendFileSuccessMessage(int requestId, int position) {
        LeanCloudUtils.printLog("--------LeanCloudHandler----------sendFileSuccessMessage---------");
    ...
    public void sendFileFailureMessage(int requestId, AVException e, int position) {
        LeanCloudUtils.printLog("--------LeanCloudHandler----------sendFileFailureMessage---------");
        e = LeanCloudUtils.getExceptionWithMessage(e);
